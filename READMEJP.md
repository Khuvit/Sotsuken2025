# RV32I 教育用 CPU - 最小シミュレーション

## これについて
このリポジトリは、Verilog で実装された小規模な教育用 RV32I（RISC-V 32ビット）CPU の例です。  
1つのファイルにまとまったコンパクトなコア（`rv32i.v`）と、単純なメモリモデル、レジスタファイル、ALU、そして32ビットワードをレジスタ x1 にロードする小さなプログラムを実行するテストベンチを含みます。
...
- `i_mem.v` - 命令メモリ（バイト配列をリトルエンディアンの32ビットワードに構成）
- `d_mem.v` - データメモリ（バイト配列をリトルエンディアンで構成し、バイト／ハーフ／ワード単位のストアに対応）
...
- Icarus Verilog（`iverilog`, `vvp`）がインストールされていること。  
  Windows の MSYS2/MinGW64 環境では `pacman` でインストール可能。  
  GTKWave は波形の閲覧に推奨されます。
